...
module_coupled_one_fit_ui <- function(id) {
    ...
    tagList(
        ...
        fluidRow(
            bs4Card(
                ...
                fluidRow(
                    column(width = 6,
                           numericInput(NS(id, "logN0_guess"), "logN0 (log CFU/g)", 2)
                    ),
                    column(width = 6,
                           awesomeCheckbox(NS(id, "logN0_fixed"), "Fixed?", FALSE)
                    )
                ),
                fluidRow(
                    column(width = 6,
                           numericInput(NS(id, "logNmax_guess"), "logNax (log CFU/g)", 8)
                    ),
                    column(width = 6,
                           awesomeCheckbox(NS(id, "logNmax_fixed"), "Fixed?", FALSE)
                    )
                ),
                fluidRow(
                    column(width = 6,
...
module_coupled_one_fit_server <- function(id) {
    ...
    moduleServer(id, function(input, output, session) {
        ...
        observeEvent(input$show_help, {
            ...
            showModal(
                session = session,
                modalDialog(title = "Fitting the Baranyi-Ratkowsky model in a one-step", 
                            easyClose = TRUE,
                            size = "xl",
                            tagList(
                                p(
                                    paste(
                                        ...
                                        "from the secondary model (Tmin, b and logC0), plus 2 parameters from the primary",
                                        "model (logN0, logNmax)."
                                    )
                                ),
                                p(
                                    paste(
                                        "The module uses a one-step approach Therefore, it estimates the secondary models directly from logN",
                                        "A two-step approach (where the primary models have already been fitted) is available in a different module."
                                    )
                                ),
                                h3("Uploading data"),
                                p(
                                ...
                                tags$li("And 'time' with the elapsed time."),
                                tags$li("And 'logN' with the observed (log10) microbial concentration."),
                                p(
                                    tagList(
        ...
        output$downloadData <- downloadHandler(
            filename = function() {
                "example_coupled_onestep.xlsx"
            },
            content = function(file) {
                file.copy("example_coupled_onestep.xlsx", file)
            }
        )
        ...
        output$hot = renderRHandsontable({
            if (!is.null(input$hot)) {
                ...
                DF <- example_coupled_onestep %>% filter(temp < 19)
            }
            
            DF %>%
                set_names(c("time", "temp", "logN")) %>%
                rhandsontable() %>%
        ...
        output$plot_guess <- renderPlot({
            ...
            p <- c(logN0 = input$logN0_guess, logNmax = input$logNmax_guess, 
                   logC0 = input$logC0_guess, b = input$b_guess, Tmin = input$Tmin_guess)
            
            my_data() %>%
                split(.$temp) %>%
                map(.,
                    ~ tibble(temp = as.numeric(.$temp[1]),
                             time = seq(0, max(.$time, na.rm = TRUE), length = 100),
                             logN = biogrowth:::pred_coupled_baranyi(p, temp, time))
                    ) %>%
        ...
        my_fit <- eventReactive(input$fit_model, {
            ...
            if (input$logN0_fixed) {
                known <- c(known, logN0 = input$logN0_guess)
            } else {
                guess <- c(guess, logN0 = input$logN0_guess)
            }
            
            if (input$logNmax_fixed) {
                known <- c(known, logNmax = input$logNmax_guess)
            } else {
                guess <- c(guess, logNmax = input$logNmax_guess)
            }
...
test_module_coupled_one_fit <- function(id) {
    ...
    shinyApp(
        ui = dashboardPage(
            ...
            body = dashboardBody(
                module_coupled_one_fit_ui("test")
            )
        ),
        server = function(input, output) {
            module_coupled_one_fit_server("test")
        }
    )
